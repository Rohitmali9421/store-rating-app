...
import { FaPlus, FaStar, FaArrowUp, FaArrowDown, FaExclamationTriangle, FaSync, FaSort, FaUsers } from 'react-icons/fa';
import Pagination from './Pagination'; // Assuming Pagination.js is in the same folder
...
const UserManagement = () => {
  ...
  useEffect(() => {
    ...
    }, 500); // Debounce to prevent rapid API calls while typing
    ...
    <div className="container mx-auto p-4 md:p-6">
            ...
            <form onSubmit={handleSubmit(onUserSubmit)} className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input {...register('name', { required: 'Name is required' })} placeholder="Full Name" className="w-full p-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500" />
                    ...
                    <div>
                        ...
                        <input {...register('email', { required: 'Email is required', pattern: { value: /^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}$/i, message: "Invalid email address" }})} placeholder="Email" className="w-full p-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500" />
                    ...
                    <div>
                        ...
                        <input type="password" {...register('password', { required: 'Password is required', minLength: { value: 6, message: "Password must be at least 6 characters" }})} placeholder="Password" className="w-full p-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500" />
                    ...
                    <div>
                        ...
                        <input {...register('address', { required: 'Address is required' })} placeholder="Address" className="w-full p-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500" />
                    ...
                    <div className="md:col-span-2">
                        ...
                        <select {...register('role', { required: 'Role is required' })} className="w-full p-2 border border-gray-300 rounded-md bg-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500" defaultValue="">
      ...
      <div className="mb-6 p-6 bg-white rounded-xl shadow-md border border-gray-200">
        <div className="flex justify-between items-center mb-4">
            ...
            <button onClick={clearFilters} className="text-sm text-blue-500 hover:text-blue-700 flex items-center gap-1">
                Clear all filters
            </button>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Role</label>
                <select name="role" value={filters.role} onChange={handleFilterChange} className="w-full p-2 border border-gray-300 rounded-md bg-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                    <option value="">All Roles</option>
                    <option value="NORMAL_USER">Normal User</option>
                    <option value="SYSTEM_ADMIN">System Admin</option>
                    <option value="SYSTEM_ADMIN">Store Owner</option>
                </select>
            </div>
        </div>
    </div>
        ...
        <div className="bg-white rounded-xl shadow-md p-8 text-center">
            <div className="flex flex-col items-center justify-center py-10">
                ...
                <p className="text-gray-600 mb-6 max-w-md">
                    {Object.values(filters).some(val => val !== '') ? 'Try adjusting your filters to find what you are looking for.' : 'There are no users in the system yet.'}
                </p>
                {Object.values(filters).some(val => val !== '') && (
                    <button onClick={clearFilters} className="bg-blue-600 hover:bg-blue-700 text-white font-medium py-2 px-4 rounded-lg">
                        Clear Filters
                    </button>
                )}
            </div>
          ...
          <div className="bg-white rounded-xl shadow-md overflow-hidden mb-6">
              ...
              <table className="w-full">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th className="p-4 text-left font-semibold text-gray-600 text-sm uppercase"><button onClick={() => requestSort('address')} className="flex items-center gap-1 font-medium hover:text-gray-800 transition-colors group">Address <span className="transition-transform group-hover:scale-110">{getSortIcon('address')}</span></button></th>
                    <th className="p-4 text-left font-semibold text-gray-600 text-sm uppercase"><button onClick={() => requestSort('role')} className="flex items-center gap-1 font-medium hover:text-gray-800 transition-colors group">Role <span className="transition-transform group-hover:scale-110">{getSortIcon('role')}</span></button></th>
                    <th className="p-4 text-center font-semibold text-gray-600 text-sm uppercase">Store Rating</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-100">
                  {users.map((user, index) => (
                    <tr key={user.id} className={`transition-colors group ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50'} hover:bg-blue-50`}>
                      <td className="p-4">
                        <div className="flex items-center">
                          <div className="w-8 h-8 bg-indigo-100 rounded-full flex items-center justify-center text-indigo-600 font-bold mr-3 group-hover:bg-indigo-200 transition-colors">{user.name.charAt(0)}</div>
                          <span className="font-medium text-gray-800">{user.name}</span>
                        </div>
                      </td>
                      <td className="p-4 text-gray-600"><div className="max-w-[200px] truncate" title={user.email}>{user.email}</div></td>
                      <td className="p-4 text-gray-600"><div className="max-w-[200px] truncate" title={user.address}>{user.address || 'Not provided'}</div></td>
                      <td className="p-4"><span className={`px-3 py-1 text-xs rounded-full font-medium ${user.role === 'SYSTEM_ADMIN' ? 'bg-purple-100 text-purple-800' : user.role === 'STORE_OWNER' ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-800'}`}>{user.role.replace('_', ' ')}</span></td>
                      <td className="p-4">
            ...
            <div className="px-4 py-3 bg-gray-50 border-t border-gray-100 text-sm text-gray-500 flex justify-between items-center">
                ...
                <div>
                    ...
                    Showing {users.length} of {userPagination?.totalItems || 0} users
                </div>
            </div>
          </div>
